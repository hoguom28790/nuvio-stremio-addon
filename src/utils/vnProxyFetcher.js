            ...
            req.on('connect', (res, socket) => {
                ...
                const client = https.get(targetUrl, {
                    ...
                    response.on('data', chunk => {
                        body += chunk;
                        // M3U8 files are at most 200KB; don't read unbounded data
...
async function fetchM3u8ViaVnProxy(targetUrl) {
    // Collect all candidate proxies (dynamic + default)
    ...
    // Try candidates in small batches of 2-3 in parallel for lowest latency
    const batchSize = 3;
